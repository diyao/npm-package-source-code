...
class Master extends EventEmitter {
  ...
  constructor(options) {
    super();
    // 有一些默认的设置，甚至可以不传参。直接调用startCluster
    // frameworkPath,从package.json中的egg.framework字段中读取
    this.options = parseOptions(options);
    // 管理agent和worker进程。启动过程用。
    this.workerManager = new Manager();
    // master进程的消息管理对象，和master统一进程。它通过事件发射和master通信。
    ...
    // logger的一些方法如info，error来自其继承的类上定义的。
    ...
    // 注册回调，参数函数加入回调队列。等待ready(true|flash|err)触发。此回调函数将接受到err参数。
    // 在onAppStart里调用了这些回调。
    this.ready(() => {
      this.isStarted = true;
      ...
      // 校验agent，worker进程是否存在。不存在有个exception事件发射
      if (this.isProduction) {
    ...
    // master收到agent-start消息后，fork其他工作进程。仅一次once。
    ...
    detectPort((err, port) => {
      ...
      if (err) {
        ...
        // 需要检测的端口不能监听，报错，退出。这里是随机监听，不可能触发。
        process.exit(1);
      }
      // 检测没有传递端口，则是随机监听一个返回。
      this.options.clusterPort = port;
      // forkAgent进程。可查阅egg官网agent和work进程fork过程。
      this.forkAgentWorker();
    });

    // exit when agent or worker exception
    // agent和worker进程状态存在异常。
  ...
  forkAgentWorker() {
    ...
    // agentworker进程被master给fork出。
    // 运行agent_worker.js文件。在其ready回调中，发射agent-start事件给master进程。
    // 往上翻阅master监听到agent-start后的逻辑。
  ...
  forkAppWorkers() {
    ...
    // master进程fork其他工作进程。count是进程数。
    // app进程是cluster模块fork出来的，agent是child_process.fork出来的。不同。
    // https://cnodejs.org/topic/56e84480833b7c8a0492e20c 参考。
    // cfork里有重启app-work逻辑。
    cfork({
    ...
    // 所欲app工作进程启动完毕，向master发通知。app-start
  ...
  onAppExit(data) {
    ...
    if (this.isAllAppWorkerStarted) {
      ...
      // 启动期间挂了，不再启动。因为很有可能是自身bug，再启动会导致循环重启。
