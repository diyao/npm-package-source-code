...
class Logger extends Map {
   ...
   */
  log(level, args, meta) {
    // 取出某个类型map如[info,error,warn,debug]，如error。其又是一个map
    const redirectLogger = this.redirectLoggers.get(level);
    if (redirectLogger) {
      redirectLogger.log(level, args, meta);
      return;
    }
    // 把把以上取出的map轮询（说明是可以扩展的）一遍，每个实例都调用其shouldLog/log
    // 官网给出了高级自定义日志 https://eggjs.org/zh-cn/core/logger.html#%E9%AB%98%E7%BA%A7%E8%87%AA%E5%AE%9A%E4%B9%89%E6%97%A5%E5%BF%97
    for (const transport of this.values()) {
      if (transport.shouldLog(level)) {
        // shouldLog在父辈类上有，可默认。但是log，一般需要扩展放自定义逻辑。
