...
class EggLoader {
   ...
   * @since 1.0.0
   */
  // loader一个文件，如果它导出一个函数，会调用并把后续参数传入，默认this.app
  // 比如加载router.js文件，参数是app。config的参数则是appInfo，见./mixin/config.js
  ...
  getLoadUnits() {
    ...
    // framework or egg path，然后加载框架。越底层的在eggPaths前头，优先加载。
  ...
  getTypeFiles(filename) {
    // 返回某个（配置）文件其所有和环境相关的文件数组。x.default，x.prod
